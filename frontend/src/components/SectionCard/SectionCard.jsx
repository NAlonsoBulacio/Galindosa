      ...
      {sectionProps ? (
        ...
        >
          <div className="w-full lg:w-[43%] flex flex-col justify-between items-start px-2 gap-y-4">
            ...
            <p className="text-sm lg:text-md poppins-light">{sectionProps.text}</p>
            <div className="flex flex-wrap justify-start gap-8 mt-4">
              {sectionProps.amenities?.map((amenity, index) => (
                <div key={index} className="flex flex-col items-center w-[18%] px-2">
                  ...
                  <p className="text-xs lg:text-md text-black text-center">{amenity.label}</p>
                </div>
              ))}
            </div>
            <a
          ...
          <div className={`${isFullscreen ? "w-screen" : "w-full lg:w-2/4"}`}>
            <div
              ref={sliderRef}
              className={`${isFullscreen ? "fixed inset-0 z-50 bg-gray-900 bg-opacity-75" : "relative"}`}
            >
              {images.length > 1 ? (
                <Slider {...settings}>
